...
func _enter_tree() -> void:  # when the extension is being enabled
	...
	var colorpicker = ExtensionsApi.general.get_global().control.find_node("Color Pickers")

	# Place the replace_button at a comfortable place
	color_change = preload("res://src/Extensions/Swappy/Elements/ColorChange.tscn").instance()
